document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', function(e){
        if(e.target.matches('.btn-convertir-lead')){
            const idpersona = e.target.dataset.id;
            abrirModalLead(idpersona);
        }
    });

    // --- Función para abrir modal de lead ---
    async function abrirModalLead(idpersona){
        try {
            const res = await fetch(`${base_url}leads/modalCrear/${idpersona}`);
            ...
            // Opcional: manejar submit del modal de lead con fetch
